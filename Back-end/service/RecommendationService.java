...
import com.twitch.twitch.dao.FavoriteDao;
...
public class RecommendationService {
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                ...
                if (!favoritedItemIds.contains(item.getId())) { //点过赞就不加进结果里
                    recommendedItems.add(item);
                }
            }
        }
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {// String 是指type, List<Item> 是指每条推荐记录
        Map<String, List<Item>> recommendedItemMap = new HashMap<>();
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                recommendedItemMap.put(entry.getKey(), recommendByTopGames(ItemType.valueOf(entry.getKey()), topGames)); //如果该类别没有点赞则使用默认
            } else {

...
import com.twitch.twitch.entity.response.Game;
import com.twitch.twitch.entity.db.Item;
import com.twitch.twitch.entity.db.ItemType;
...
public class GameService {
    private static final String TOKEN = "Bearer idmd96xv8ehknmmlbrpt1i2pck5ugl";
    private static final String CLIENT_ID = "s669l58veewvjmkxz3ah3kj4bs0j38";
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            ...
            return String.format(url, gameName); //string function, 替换%s
        }
    }
    ...
    // Send the HTTP request to Twitch Backend based on the given URL, and returns the body of the HTTP response returned from Twitch backend.
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = response -> {
            ...
            if (entity == null) { //request成功但是没有内容
                throw new TwitchException("Failed to get result from Twitch API");
            }
        ...
        try {
            ...
            return httpclient.execute(request, responseHandler); //执行之后，call handler
        } catch (IOException e) { //比如server被拔网线
            e.printStackTrace();
            throw new TwitchException("Failed to get result from Twitch API");
        } finally {
            try {
    ...
    private List<Game> getGameList(String data){
        ObjectMapper mapper = new ObjectMapper(); //mapper把json array string转成java obj
        try {
            return Arrays.asList(mapper.readValue(data, Game[].class)); //把data转换成Game类型的array，再转成list
